...
async def ocr(data: dict[str, str], *, ocr_client: Client, **param) -> OcrResult:
    match data:
        case {'httpurl': url}:
            return ocr_client.parse_result(await ocr_client.ocr_by_httpurl(url, **param))
        case {'base64image': image}:
            return ocr_client.parse_result(await ocr_client.ocr_by_base64image(image, **param))
        case _:
